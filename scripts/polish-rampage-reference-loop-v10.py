...
def apply_rampage_reference_loop_v10(target: Path) -> None:
   ...
   const sample=u=>{
    if(u<=entryEnd){const q=clamp(u/entryEnd,0,1),seed=mixV(startFrame.up,ringEntry.up,smooth01(q)),base=hermiteOpen(startFrame.p,startFrame.forward,ringEntry.pos,entryT,entryScale,entryScale,q),pos=add(base,mul(loopForward,-throatOpen*throatBump(q)));return{pos,frame:{up:seed},upSeed:seed};}
    if(u>=exitStart){const q=clamp((u-exitStart)/(1-exitStart),0,1),seed=mixV(ringExit.up,endFrame.up,smooth01(q)),base=hermiteOpen(ringExit.pos,exitT,endFrame.p,endFrame.forward,exitScale,exitScale,q),pos=add(base,mul(loopForward,throatOpen*throatBump(1-q)));return{pos,frame:{up:seed},upSeed:seed};}
