...
export function buildRaceTrack({box,sign}){
 ...
 // High start arch: the banner's local X axis must follow the road right vector.
 // The previous +PI/2 rotation turned the 20m plate along the road and stabbed
 // through the chase camera. Keep it transverse, high, and just ahead of spawn.
 const gantryS=14,start=racePointAt(gantryS);for(const side of [-1,1]){const p=racePointAt(gantryS,side*(W+1.2));box(group,p.x,4.0,p.z,.42,8.0,.42,0x8a938e,true);}const beam=box(group,start.x,start.y+7.55,start.z,W*2+4.6,.46,.50,P.edge,true);beam.rotation.y=start.heading;const banner=sign(spec.name,W*2,1.25);banner.position.set(start.x,start.y+6.65,start.z);banner.rotation.y=start.heading;group.add(banner);
