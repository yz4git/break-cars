...
def apply_rampage_opening_toast_v20(target: Path) -> None:
    ...
    new = "if(count<=0){mode='race';if(world.mode==='racing'&&activeCourse.id==='rampage-3d')toast('RAMPAGE 3D — OMEGAへ',1.15);else toast(world.mode==='racing'?'RAMPAGE 3D！ 飛べ、ぶつけろ、ループを抜けろ。':world.mode==='wreck-hunt'?'HUNT START — WRECK TARGETS':'側面を狙え。生き残れ。',2);}"
