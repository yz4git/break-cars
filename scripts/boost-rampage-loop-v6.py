...
the steep ASCENDING leg and gravity compensation remains ascent-only. Around
the crown we damp off-tangent velocity like a loaded tyre. From the upper arc
through the authored loop exit, a mostly perpendicular steering force rotates
velocity toward the road tangent instead of dissipating its energy. The lower
exit uses a short speed-scaled tangent look-ahead for both steering and drive
force so the physical response starts before track projection advances into the
rapidly flattening road. The exit target still tapers down and overspeed trim
remains in force.
