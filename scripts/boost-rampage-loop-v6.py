...
The longer natural lead-in gets a slightly higher pre-loop target so the car
arrives at the first loop sample with the same physical speed margin as before.
The legacy no-query race harness resolves to the classic course selection while
still constructing RAMPAGE geometry, so classic is included only as that legacy
RAMPAGE compatibility path.
"""
