...
open/twisted loop changes its tangent more progressively than the old closed
circle, so the boost reacts more strongly to short-lived forward-speed loss
while staying capped by a controlled target speed.  While physically inside
the loop, the strip also keeps applying through brief suspension-unloaded
moments at the inverted crown.
"""
