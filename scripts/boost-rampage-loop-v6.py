...
boost marker, and remains force/torque-only. Normal damping, look-ahead steering,
and drive gains stay on the proven values. The only crown rescue is a small
3.2 m/s² force along the CURRENT road tangent through the short 0.43-0.50 loop
phase where the tangent rotates fastest. Unlike raising the main controller gain,
this does not pull the car toward a future tangent and therefore preserves the
established inversion and exit trajectory while restoring the missing contact
speed margin. SKY FORGE and DOUBLE ORBIT are deliberately untouched.
"""
