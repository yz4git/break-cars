...
approach target is raised to preserve a generous entry-speed margin for the
photo-matched open-leg loop, while the lower/middle loop target stays around
106 km/h.  The final quarter of the loop gently tapers toward about 94 km/h so
the car clears the stunt with margin without launching off the following
course. Cars slowed to zero or nudged slightly backward are also recovered by
the strip. While physically inside the loop, the strip keeps applying its
forward force through brief suspension-unloaded moments at the inverted crown
so a taller/open loop cannot lose the assist for a few frames.
"""
