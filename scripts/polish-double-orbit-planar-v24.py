...
"""
    ...
    # Stage the active DOUBLE ORBIT ring independently of the older single-loop
    # racingLoop selector. Outside DOUBLE ORBIT, the proven RAMPAGE v18 block is
    # left byte-for-byte intact and still owns the generic racingLoop branch.
    game = target / 'game.js'
    s = game.read_text()
    ...
    double_camera = """const doubleOrbitLoop=activeCourse?.id==='double-orbit'?(fullPhysicsSpec.race?.loops||[]).find(q=>(p.trackS??0)>=q.startS-6&&(p.trackS??0)<=q.endS+6):null;if(doubleOrbitLoop){const s0=doubleOrbitLoop.startS,s1=doubleOrbitLoop.endS,a=trackPoint(s0,0),z=trackPoint(s1,0),mx=(a.x+z.x)*.5,my=(a.y+z.y)*.5,mz=(a.z+z.z)*.5,rd=Math.hypot(a.right.x,a.right.z)||1,rx=a.right.x/rd,rz=a.right.z/rd,fd=Math.hypot(a.forward.x,a.forward.z)||1,fx=a.forward.x/fd,fz=a.forward.z/fd,outward=(mx*rx+mz*rz)>=0?1:-1,side=(view===1?2.85:2.55)*doubleOrbitLoop.radius*outward,back=(view===1?1.55:1.28)*doubleOrbitLoop.radius,stageLift=view===1?2.8:2.1,loopFocusY=my+doubleOrbitLoop.radius-1.35,playerFocus=.30;camTarget.set(mx+rx*side-fx*back,my+doubleOrbitLoop.radius+stageLift,mz+rz*side-fz*back);lookTarget.set(mx+(b.px-mx)*playerFocus,loopFocusY+(b.py+.45-loopFocusY)*playerFocus,mz+(b.pz-mz)*playerFocus);camera.up.lerp(physicsWorldUp,1-Math.exp(-12*dt));camera.fov=view===1?56:57;}else if(racingLoop&&activeCourse?.id!=='double-orbit'){const s0=raceLoopSpec.startS,s1=raceLoopSpec.endS,a=trackPoint(s0,0),z=trackPoint(s1,0),mx=(a.x+z.x)*.5,my=(a.y+z.y)*.5,mz=(a.z+z.z)*.5,dx=z.x-a.x,dz=z.z-a.z,dl=Math.hypot(dx,dz)||1,nx=dx/dl,nz=dz/dl,fx=a.forward.x,fz=a.forward.z,outward=(mx*nx+mz*nz)>=0?1:-1,side=-(view===1?2.64:2.45)*raceLoopSpec.radius*outward,back=(view===1?2.17:2.00)*raceLoopSpec.radius,stageLift=view===1?3.5:2.7,loopFocusY=my+raceLoopSpec.radius-2.6,playerFocus=.24;camTarget.set(mx+nx*side-fx*back,my+raceLoopSpec.radius+stageLift,mz+nz*side-fz*back);lookTarget.set(mx+(b.px-mx)*playerFocus,loopFocusY+(b.py+.45-loopFocusY)*playerFocus,mz+(b.pz-mz)*playerFocus);camera.up.lerp(physicsWorldUp,1-Math.exp(-12*dt));camera.fov=view===1?54:55;}"""
