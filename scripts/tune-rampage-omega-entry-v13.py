...
def apply_rampage_omega_entry_v13(target: Path) -> None:
    ...
    s = replace_once(
        ...
        "guideForward=curveGuide?norm(add(mul(road.forward,exitBlend?.42:entryTransition?.88:.60),mul(ahead.forward,exitBlend?.58:entryTransition?.12:.40))):road.forward",
        'entry tangent blend',
    )
    s = replace_once(
        s,
        "guide=crownBand?(b.groundedWheels===0?12.0:10.2):contactArc?(exitBlend?2.0:8.4):6.2",
        "guide=entryTransition?21.5:crownBand?(b.groundedWheels===0?12.0:10.2):contactArc?(exitBlend?2.0:8.4):6.2",
        'entry cross-velocity damping',
    )
    s = replace_once(
        s,
        "turnBase=exitBlend?26:12,turnScale=exitBlend?1.55:1.05,turnCap=exitBlend?110:70",
        "turnBase=exitBlend?26:entryTransition?27:12,turnScale=exitBlend?1.55:entryTransition?1.42:1.05,turnCap=exitBlend?110:entryTransition?104:70",
