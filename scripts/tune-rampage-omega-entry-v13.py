...
The planar-Omega geometry is intentionally left untouched.  The new lower C2
leg changes tangent more quickly than the old fully twisted ribbon, so a fast
car can carry a large cross-tangent velocity component into the first part of
the loop.  This pass only changes force guidance in that first 20% of RAMPAGE.
...
def apply_rampage_omega_entry_v13(target: Path) -> None:
    ...
    s = replace_once(
        ...
        "guideForward=curveGuide?norm(add(mul(road.forward,exitBlend?.42:entryTransition?.78:.60),mul(ahead.forward,exitBlend?.58:entryTransition?.22:.40))):road.forward",
        'entry tangent blend',
    )
    s = replace_once(
        s,
        "guide=crownBand?(b.groundedWheels===0?12.0:10.2):contactArc?(exitBlend?2.0:8.4):6.2",
        "guide=entryTransition?16.0:crownBand?(b.groundedWheels===0?12.0:10.2):contactArc?(exitBlend?2.0:8.4):6.2",
        'entry cross-velocity damping',
    )
    s = replace_once(
        s,
        "turnBase=exitBlend?26:12,turnScale=exitBlend?1.55:1.05,turnCap=exitBlend?110:70",
        "turnBase=exitBlend?26:entryTransition?22:12,turnScale=exitBlend?1.55:entryTransition?1.25:1.05,turnCap=exitBlend?110:entryTransition?88:70",
