...
let loopInverted=false,loopExited=false,jumpAir=0,jumpLanded=false,bridgeSeen=false,recover=0,auto=0,maxSJump=0,prevS=p.trackS,longestStuck=0,stuck=0,frames=0;
...
// First-loop pack regression. RAMPAGE is full-contact, but the opening stunt
// must not destroy most of the field before the actual race develops.
const pack=makeWorld(0,144,'racing');pack.endAt=999;pack.limit=999;pack.done=false;
let packMaxSJump=0,packPrev=pack.cars.map(c=>c.trackS),maxLoopCars=0,packImpacts=0;
for(let frame=0;frame<660&&!pack.done;frame++){
  step(pack,{},1/60,true);
  let loopCars=0;
  for(const c of pack.cars){
    ...
    if(racePointAt(c.trackS).kind==='loop')loopCars++;
  }
...
console.log(`RAMPAGE raceability OK: solo ${(p.raceDistance-start).toFixed(1)}/${spec.length.toFixed(1)}m in ${(frames/60).toFixed(1)}s, loopInverted=${loopInverted}, jumpAir=${jumpAir.toFixed(2)}, pack=${survivors}/12, loopCars=${maxLoopCars}, impacts=${packImpacts}`);
