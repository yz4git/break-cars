...
replace_once(
    ...
    "wreckSide:c.id%2?1:-1,wreckTilt:.24}};}",
    'reaction state',
)
...
replace_once(
    ...
    "if(c.dead){const targetRoll=rxn.wreckSide*rxn.wreckTilt;rxn.roll+=(targetRoll-rxn.roll)*(1-Math.exp(-2.35*dt));rxn.pitch+=(-.13-rxn.pitch)*(1-Math.exp(-2.1*dt));}",
    'wreck target angle',
)
