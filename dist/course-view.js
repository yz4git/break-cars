...
export function buildCourseTerrain(){
 ...
 for(let j=0;j<=n;j++)for(let i=0;i<=n;i++){const x=(i/n-.5)*size,z=(j/n-.5)*size,y=courseHeight(x,z);verts.push(x,y+.015,z);const stripe=Math.floor((activeCourse.id==='crater-crown'?Math.hypot(x,z):z+45)/3)%2,c=new THREE.Color(activeCourse.id==='crater-crown'?(stripe?0x807461:0x696457):(stripe?0x55767b:0x344f59));if(activeCourse.id==='maelstrom-pit')c.setHex(y>5?(stripe?0xff9346:0x8e4330):(stripe?0x493d53:0x302d3e));if(activeCourse.id==='cross-fire')c.setHex(y>4?(stripe?0xffd666:0x3c3943):(stripe?0x31586c:0x20374a));c.multiplyScalar(.8+Math.min(y,5)*.07);colors.push(c.r,c.g,c.b);}
