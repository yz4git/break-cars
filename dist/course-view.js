...
export function buildCourseTerrain(){
 ...
 for(let j=0;j<=n;j++)for(let i=0;i<=n;i++){const x=(i/n-.5)*size,z=(j/n-.5)*size,y=courseHeight(x,z);verts.push(x,y+.015,z);const stripe=Math.floor(Math.hypot(x,z)/3)%2,c=new THREE.Color(activeCourse.id==='crater-crown'?(stripe?0x807461:0x696457):0x526a70);c.multiplyScalar(.8+y*.07);colors.push(c.r,c.g,c.b);}
