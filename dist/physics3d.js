...
// Works reconstruction: 6DoF rigid bodies, gravity, 4-wheel suspension,
// 3D ride-over contacts, uneven ground, jump ramp and physical vertical loop.
export const FULL_PHYSICS_ID = 'works-full-physics3d-v4';
...
function baseHeight(x,z) {
  ...
  for (const [bx,bz,amp,rad] of BUMPS) {
    ...
    const e=Math.exp(-d2/rr*2.2),v=amp*e; h+=v; gx+=v*(-4.4*dx/rr); gz+=v*(-4.4*dz/rr);
  }
...
function loopSurface(x,y,z) {
  ...
  const dy=y-LOOP.y,dz=z-LOOP.z,r=Math.hypot(dy,dz); if (r<EPS) return null;
...
function syncLegacy(c) {
  const b=c.p3,f=bodyForward(b),flat=Math.hypot(f.x,f.z); c.x=b.px; c.z=b.pz; c.vx=b.vx; c.vz=b.vz; if (flat>.08) c.heading=Math.atan2(f.x,f.z); c.omega=b.wy;
  b.lastSyncX=c.x; b.lastSyncZ=c.z; b.lastSyncVx=c.vx; b.lastSyncVz=c.vz;
}
function adoptExternal(c,types) {
  const b=c.p3;
...
function wheelForces(w,c,u,ctx,dt,acc) {
  ...
  const top=(player ? (rush ? 38+Math.min(w.hunt?.combo||0,4)*1.4 : 33.5) : 29)*damageFactor;
  let steer=c.dead ? 0 : (u.steer||0); if (player) steer=-steer;
  if (player&&w.mode==='wreck-hunt'&&!c.dead) {
    const focus=w.cars[w.hunt?.focusId];
    if (focus&&!focus.dead) { const desired=Math.atan2(focus.x-c.x,focus.z-c.z),assist=ctx.clamp(ctx.angle(desired-c.heading)*.42,-.34,.34)*(1-ctx.clamp(Math.abs(u.steer||0),0,1)*.80); steer=ctx.clamp(steer+assist,-1,1); }
    const rd=Math.hypot(c.x,c.z),edge=ctx.clamp((rd-(ctx.RADIUS-11))/6,0,1);
    if (edge>0) { const desired=Math.atan2(-c.x,-c.z),rescue=ctx.clamp(ctx.angle(desired-c.heading)*.78,-1,1); steer=ctx.clamp(steer+rescue*edge*(Math.abs(u.steer||0)<.55 ? 1 : .40),-1,1); }
  }
  c.steer=steer; c.throttle=c.dead ? 0 : (u.gas||0);
  const forward=bodyForward(b),forwardSpeed=dot({x:b.vx,y:b.vy,z:b.vz},forward),reverse=player&&w.mode==='wreck-hunt' ? 21 : player&&w.mode!=='racing' ? 17 : 12;
  let drive=(c.dead ? 0 : (u.gas||0))*power*damageFactor;
  if (!c.dead&&u.brake) drive-=forwardSpeed>1 ? (player ? 36 : 33) : reverse;
  if (forwardSpeed>top&&drive>0) drive=0;
  b.grounded=false; b.groundedWheels=0; const spring=95*b.mass,damper=8.5*b.mass;
  for (let i=0;i<WHEELS.length;i++) {
    const [x,y,z]=WHEELS[i],a=pointWorld(b,x,y,z),s=samplePhysicsSurface(w.mode,a.x,a.y,a.z,up,false),gap=dot(sub(a,s.point),s.normal)-WHEEL_RADIUS,compression=ctx.clamp(SUSPENSION_REST-gap,0,.34);
    b.wheelCompression[i]=compression; b.wheelNormal[i]=0; if (compression<=0) continue;
    ...
    const wa=i<2 ? steer*.48 : 0,sn=Math.sin(wa),cs=Math.cos(wa),wf=qrot(q,{x:sn,y:0,z:cs}),wr=qrot(q,{x:cs,y:0,z:-sn});
    let ft=sub(wf,mul(s.normal,dot(wf,s.normal))),rt=sub(wr,mul(s.normal,dot(wr,s.normal))); norm(ft); norm(rt);
    const vl=dot(pv,ft),vs=dot(pv,rt),maxF=Math.max(5,normal*(u.hand ? 1.15 : 2.25)),latScale=u.hand ? .32 : 1;
    const lateral=ctx.clamp(-vs*grip*b.mass*latScale,-maxF,maxF); let longitudinal=drive/4-vl*(u.hand ? 1 : .22)*b.mass;
    if (u.hand) longitudinal+=ctx.clamp(-vl*4*b.mass,-maxF*.7,maxF*.7); longitudinal=ctx.clamp(longitudinal,-maxF,maxF); addForce(acc,add(mul(ft,longitudinal),mul(rt,lateral)),a.r);
  }
...
function chassisContact(w,c,ctx) {
  ...
  for (const x of [-BODY_HALF.x,BODY_HALF.x]) for (const y of [-BODY_HALF.y,BODY_HALF.y]) for (const z of [-BODY_HALF.z,BODY_HALF.z]) {
    const p=pointWorld(b,x,y,z),s=samplePhysicsSurface(w.mode,p.x,p.y,p.z,up,true),d=dot(sub(p,s.point),s.normal); if (d<0&&(!deepest||d<deepest.d)) deepest={p,s,d};
  }
  if (!deepest) return; const n=deepest.s.normal,pen=-deepest.d; b.px+=n.x*pen*.72; b.py+=n.y*pen*.72; b.pz+=n.z*pen*.72;
  const pv=pointVelocity(b,deepest.p.r),vn=dot(pv,n);
  if (vn<0) { impulse(b,mul(n,-vn/Math.max(EPS,b.invMass)*.78),deepest.p.r); if (Math.abs(vn)>4&&w.time-c.hitAt>.32) { ctx.hit(w,c,(Math.abs(vn)-3)*.30,n.x,n.z); w.events.push({type:'wall',car:c.id,x:b.px,z:b.pz,power:Math.abs(vn),nx:n.x,nz:n.z}); } }
}
function arenaBoundary(w,c,ctx) {
  if (w.mode==='racing') return; const b=c.p3,r=Math.hypot(b.px,b.pz),limit=ctx.RADIUS-2.45; if (r<=limit) return;
  const nx=b.px/(r||1),nz=b.pz/(r||1),pen=r-limit; b.px-=nx*pen; b.pz-=nz*pen; const v=b.vx*nx+b.vz*nz;
  if (v>0) { b.vx-=nx*v*1.45; b.vz-=nz*v*1.45; impulse(b,{x:-nx*Math.min(v*.24,3.2),y:0,z:-nz*Math.min(v*.24,3.2)},{x:nx*BODY_HALF.x,y:0,z:nz*BODY_HALF.x}); if (v>5&&w.time-c.hitAt>.35) { ctx.hit(w,c,(v-4)*.55,nx,nz); w.events.push({type:'wall',car:c.id,x:b.px+nx,z:b.pz+nz,power:v,nx,nz}); } }
}
function raceBoundary(w,c,ctx) {
  if (w.mode!=='racing') return; syncLegacy(c); const ox=c.x,oz=c.z,wall=ctx.constrainTrack(c),b=c.p3; if (c.x!==ox||c.z!==oz) { b.px=c.x; b.pz=c.z; }
...
function vehiclePair(w,a,b,ctx) {
  ...
  if (!contacts.length) return; contacts.sort((x,y)=>y.over-x.over);
  const inv=A.invMass+B.invMass,as=A.invMass/inv,bs=B.invMass/inv,rush=w.mode==='wreck-hunt'&&(a.id===0||b.id===0)&&w.hunt?.rushUntil>w.time; let peak=0,pn=contacts[0].n;
  for (const ct of contacts.slice(0,3)) {
    const n=ct.n,cor=ct.over*.34/Math.min(3,contacts.length); A.px-=n.x*cor*as; A.py-=n.y*cor*as; A.pz-=n.z*cor*as; B.px+=n.x*cor*bs; B.py+=n.y*cor*bs; B.pz+=n.z*cor*bs;
    const closing=dot(sub(pointVelocity(A,ct.pa.rel),pointVelocity(B,ct.pb.rel)),n); if (closing<=0) continue; if (closing>peak) { peak=closing; pn=n; }
    const j=closing*(rush ? 1.15 : 1)/Math.max(EPS,inv)*.46,J=mul(n,j); impulse(A,mul(J,-1),ct.pa.rel); impulse(B,J,ct.pb.rel);
  }
  if (peak<=0) return;
  const key=a.id*ctx.COUNT+b.id,cool=w.mode==='racing' ? .32 : .42,min=w.mode==='racing' ? 2.5 : 3; if (peak<min||w.time-(w.pairs.get(key)??-10)<cool) return; w.pairs.set(key,w.time);
  const h=Math.hypot(pn.x,pn.z)||1,nx=pn.x/h,nz=pn.z/h,ma=ctx.TYPES[a.type].mass*(a.id===0 ? 1.12 : 1),mb=ctx.TYPES[b.type].mass*(b.id===0 ? 1.12 : 1),damage=(peak-2)*.42*(w.mode==='racing' ? 1.28 : 1);
  const da=ctx.hit(w,a,damage*mb,nx,nz,b)||0,db=ctx.hit(w,b,damage*ma,-nx,-nz,a)||0; adoptHitVelocity(a); adoptHitVelocity(b);
  ...
  w.events.push({type:'impact',a:a.id,b:b.id,power:peak,x:(A.px+B.px)/2,y:(A.py+B.py)/2,z:(A.pz+B.pz)/2});
}
...
function integrateBody(w,c,u,ctx,dt) {
  ...
  b.vx+=acc.fx*b.invMass*dt; b.vy+=acc.fy*b.invMass*dt; b.vz+=acc.fz*b.invMass*dt; const aa=invInertiaWorld(b,{x:acc.tx,y:acc.ty,z:acc.tz}); b.wx+=aa.x*dt; b.wy+=aa.y*dt; b.wz+=aa.z*dt;
  const ad=Math.exp(-(c.dead ? .28 : 1.05)*dt); b.wx*=ad; b.wy*=Math.exp(-(c.dead ? .32 : .72)*dt); b.wz*=ad; b.px+=b.vx*dt; b.py+=b.vy*dt; b.pz+=b.vz*dt; integrateQ(b,dt);
  chassisContact(w,c,ctx); arenaBoundary(w,c,ctx); raceBoundary(w,c,ctx); syncLegacy(c);
}
...
function maintainHunt(w,ctx) {
  ...
  if (ctx.chooseHuntFocus) { const f=w.cars[h.focusId]; if (!f||f.dead||f.id===0) h.focusId=ctx.chooseHuntFocus(w); const locked=w.cars[h.focusId]; if (locked&&!locked.dead&&h.combo>0) { locked.target=0; locked.aiTimer=Math.max(locked.aiTimer,.35); } }
...
export function stepFullPhysics(w,input,dt=1/60,autoplay=false,ctx) {
  ...
  for (const c of w.cars) { adoptExternal(c,ctx.TYPES); syncLegacy(c); }
  const controls=w.cars.map(c=>control(w,c,input,dt,autoplay,ctx));
  for (let substep=0;substep<2;substep++) {
    const h=dt*.5; for (let i=0;i<w.cars.length;i++) integrateBody(w,w.cars[i],controls[i],ctx,h);
    for (let pass=0;pass<2;pass++) for (let i=0;i<ctx.COUNT;i++) for (let j=i+1;j<ctx.COUNT;j++) vehiclePair(w,w.cars[i],w.cars[j],ctx);
