...
for(const item of cases){
 ...
 await page.click('#start');await page.waitForTimeout(4300);await page.keyboard.down('ArrowUp');await page.waitForTimeout(950);
 await page.screenshot({path:path.join(dir,'10-follow.png')});
 for(let i=0;i<4;i++){const key=i%2?'ArrowRight':'ArrowLeft';await page.keyboard.down(key);await page.waitForTimeout(520);await page.keyboard.up(key);}
 await page.screenshot({path:path.join(dir,'20-action.png')});
 await page.keyboard.press('KeyC');await page.waitForTimeout(1000);await page.screenshot({path:path.join(dir,'30-wide.png')});
 await page.keyboard.press('KeyC');await page.waitForTimeout(1000);await page.screenshot({path:path.join(dir,'40-overhead.png')});
 ...
 const row={...item,url,menu,hud,renderer,deck,errors};report.push(row);await fs.writeFile(path.join(dir,'diagnostics.json'),JSON.stringify(row,null,2));await context.close();
}
await browser.close();await fs.writeFile(path.join(out,'summary.json'),JSON.stringify(report,null,2));
const bad=report.filter(x=>x.errors.length||x.renderer==='none'||x.menu.modeSelected!=='true'||x.menu.course!==x.course||x.deck.mode!=='death-colosseum'||x.hud.scoreLabel!=='RING OUT SCORE');
console.log(`DEATH COLOSSEUM audit: ${report.length} courses, errors=${bad.length}`);
for(const x of report)console.log(`${x.course.padEnd(14)} renderer=${x.renderer} alive=${x.hud.alive.replace(/\s+/g,' ')} hp=${x.hud.hp||'-'} mode=${x.deck.mode} errors=${x.errors.length}`);
if(bad.length){for(const x of bad)console.error(`DEATH REVIEW FAIL ${x.course}: ${x.errors.join('; ')||JSON.stringify({menu:x.menu,deck:x.deck,hud:x.hud})}`);process.exitCode=1;}
